...
func (d *DFReader) rewind() {
	d.messages = map[string]*DFMessage{
		"MAV":     nil,
		"__MAV__": nil,
	}
...
func (d *DFReader) setTime(m *DFMessage) {
...
func (d *DFReader) recvMsg() DFMessage {
	return *d.parseNext()
}

func (d *DFReader) addMsg(m *DFMessage) {
	msgType := m.GetType()
